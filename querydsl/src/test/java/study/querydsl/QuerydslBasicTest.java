...
//        assertThat(teamA.get(member.age.avg())).isEqualTo()
    ...
    public void dynamicQuery_WhereParam() {
        ...
        List<Member> result = secondMember2(usernameParam, ageParam);
        assertThat(result.size()).isEqualTo(1);
    }

    private List<Member> secondMember2(String usernameCond, Integer ageCond) {
        JPAQueryFactory queryFactory = new JPAQueryFactory(em);
        return queryFactory
                .selectFrom(member)
//                .where(usernameEq(usernameCond), ageEq(ageCond))    // 응닫값이 null이면 무시된다.
    ...
    public void sqlFunction() {
        JPAQueryFactory queryFactory = new JPAQueryFactory(em);
        /*
            조회를 할 건데, 멤버의 이름이 안주인 멤버의 이름을 안주형으로 바꿔서 조회해라
         */
        List<String> result = queryFactory
                .select(
                        Expressions.stringTemplate("function('replace', {0}, {1}, {2})", member.username, "안주", "안주형"))
                .from(member)
                .fetch();
